...
static void bgmac_dma_tx_free(struct bgmac *bgmac, struct bgmac_dma_ring *ring)
{
	...
	while (ring->start != ring->end) {
		...
		if (slot->skb) {
			...
			/* Free memory! :) */
			dev_kfree_skb(slot->skb); //DD clean... 
			slot->skb = NULL;
		}
			     ...
			     int weight)
{
	...
	while (ring->start != end_slot) {
		...
		do {
			...
			napi_gro_receive(&bgmac->napi, skb);//DD indicate upper... to switch or to something else...
			handled++;
		} while (0);
...
static int bgmac_poll(struct napi_struct *napi, int weight)
{
	...
	bgmac_dma_tx_free(bgmac, &bgmac->tx_ring[0]); //DD handle tx
	handled += bgmac_dma_rx_read(bgmac, &bgmac->rx_ring[0], weight); //DD handle rx.
	...
	if (handled < weight) {
		napi_complete(napi);//DD complete and turn on interrupt again...
		bgmac_chip_intrs_on(bgmac);
	}
...
static int bgmac_open(struct net_device *net_dev)
{
	...
	err = bgmac_dma_init(bgmac);//DD get dma ready.
	if (err)
		return err;

	/* Specs say about reclaiming rings here, but we do that in DMA init */
	bgmac_chip_init(bgmac);//DD chip power on.

	err = request_irq(bgmac->core->irq, bgmac_interrupt, IRQF_SHARED,
			  KBUILD_MODNAME, net_dev);//DD hook up interrupt..
	if (err < 0) {
		bgmac_err(bgmac, "IRQ request error: %d!\n", err);
		bgmac_dma_cleanup(bgmac);
		return err;
	}
	napi_enable(&bgmac->napi);//DD all kinds of start...
...
static int bgmac_stop(struct net_device *net_dev)
{
	...
	phy_stop(bgmac->phy_dev); //DD stop..
				    ...
				    struct net_device *net_dev)
{
	...
	return bgmac_dma_tx_add(bgmac, ring, skb);//DD so simply tx...
}
...
static int bgmac_set_mac_address(struct net_device *net_dev, void *addr)
{
	...
	if (ret < 0)
		return ret;
	bgmac_write_mac_address(bgmac, (u8 *)addr);//DD mac address program
...
static const struct net_device_ops bgmac_netdev_ops = {
	.ndo_open		= bgmac_open,//DD shoudl call open first
	.ndo_stop		= bgmac_stop,
...
static int bgmac_probe(struct bcma_device *core)
{
	...
	if (!is_valid_ether_addr(mac)) { //DD get mac..
	...
	net_dev->ethtool_ops = &bgmac_ethtool_ops; //DD hook up ethtool ops..
	...
	/* Defaults */
	memcpy(bgmac->net_dev->dev_addr, mac, ETH_ALEN); //DD mac addr
	...
	bgmac->phyaddr &= BGMAC_PHY_MASK; //DD get physical address... don't know, but just remember and use it.
	...
	err = bgmac_dma_alloc(bgmac); //DD alloc tx/rx ring...
	if (err) {
	...
	if (bcm47xx_nvram_getenv("et0_no_txint", NULL, 0) == 0)//DD check nvram to configration...
		bgmac->int_mask &= ~BGMAC_IS_TX_MASK;

	/* TODO: reset the external phy. Specs are needed */
	bgmac_phy_reset(bgmac);//DD reset phy..
	...
	netif_napi_add(net_dev, &bgmac->napi, bgmac_poll, BGMAC_WEIGHT); //DD napi is used for interrupt handle.

	err = bgmac_mii_register(bgmac);//DD register MII bus
	if (err) {
	...
	if (bgmac_is_bcm4707_family(bgmac) && !bgmac_b53_pdata.regs) {
		...
		err = platform_device_register(&bgmac_b53_dev); //DD register b53 device... b53 switch comes up...
		if (!err)
			bgmac->b53_device = &bgmac_b53_dev;
	}

	err = register_netdev(bgmac->net_dev); //DD net device..
	if (err) {
...
static void bgmac_remove(struct bcma_device *core)
{
	...
	if (bgmac->b53_device)
		platform_device_unregister(&bgmac_b53_dev); //DD simply, and direct...
	bgmac->b53_device = NULL;

	unregister_netdev(bgmac->net_dev);//DD net device..
	bgmac_mii_unregister(bgmac); //DD why mii???
	netif_napi_del(&bgmac->napi);//DD what's napi???
	bgmac_dma_free(bgmac);
	bcma_set_drvdata(core, NULL);
...
static struct bcma_driver bgmac_bcma_driver = {
	...
	.probe		= bgmac_probe,  //DD also, should be called from bottom...
	.remove		= bgmac_remove, //DD remove.
};
...
static int __init bgmac_init(void)
{
	...
	err = bcma_driver_register(&bgmac_bcma_driver);//DD oh, register to bcma bus driver...
	if (err)
		return err;
...
{
	bcma_driver_unregister(&bgmac_bcma_driver);//DD unregister
}
